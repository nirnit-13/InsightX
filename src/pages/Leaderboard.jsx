 ...
 *
 * FIXES:
 *  1. Uses the merged leaderboard (real API + all original mock contributors)
 *  2. Auto-refreshes every 15 seconds so changes (add/remove contributors) appear in real time
 *  3. New contributors get simulated stats via simulateContributorStats
 *  4. Shows a "live" pulse indicator while polling
 */
...
import { CONTRIBUTORS as MOCK_CONTRIBUTORS, mergeContributors } from '../data/mockData'
import {
  RiTrophyLine, RiFireLine, RiArrowUpLine, RiArrowDownLine, RiRefreshLine,
} from 'react-icons/ri'
...
export default function Leaderboard() {
  ...
  // ── Load merged leaderboard (real API + mock originals) ───────────────────
  const loadLeaderboard = useCallback(async (showPulse = false) => {
    try {
      const data = await analyticsAPI.getLeaderboard()
      setContributors(Array.isArray(data) ? data : mergeContributors([]))
      ...
      // Fallback to original mock contributors only
  ...
  // ── Poll every 15 s so new/removed contributors appear automatically ──────
  useEffect(() => {
    const interval = setInterval(() => loadLeaderboard(true), 15_000)
        ...
        <div className="grid grid-cols-3 gap-4 mb-10">
          {[0, 1, 2].map(i => (
            <div key={i} className="flex flex-col items-center gap-3 animate-pulse">
              ...
              <div className="h-12 shimmer-bg rounded-t-lg w-full" />
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-ix-muted text-sm mt-1">
            {contributors.length} contributors ranked · May 2025
          </p>
        </div>
        <div className="flex items-center gap-2">
          {/* Live indicator */}
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-ix-green/30 bg-ix-green/10 text-ix-green text-xs font-mono">
            <motion.span
              animate={pulse ? { scale: [1, 1.4, 1] } : {}}
              transition={{ duration: 0.4 }}
              className="w-1.5 h-1.5 rounded-full bg-ix-green animate-pulse"
            />
            LIVE
          </div>
          <button
            onClick={() => loadLeaderboard(true)}
            className="w-8 h-8 flex items-center justify-center glass border border-ix-border rounded-xl text-ix-muted hover:text-ix-accent transition-all"
            title="Refresh now"
          >
          ...
          {[top3[1], top3[0], top3[2]].map((c, podiumIdx) => {
            ...
            return (
              ...
              >
                ...
                <p className="font-display font-bold text-sm text-ix-text text-center">
                  {(c.name || '').split(' ')[0]}
                </p>
                <p className="text-[10px] text-ix-muted mb-2">{c.team || 'General'}</p>
                <p className="text-xl font-display font-bold" style={{ color: c.color || '#6366f1' }}>
                  {c[sortBy] ?? 0}{sortBy === 'attendance' ? '%' : ''}
                </p>
                <p className="text-[10px] text-ix-muted capitalize mb-3">
                  {SORT_OPTIONS.find(o => o.key === sortBy)?.label}
                </p>
                <div
                  className={`w-full glass rounded-t-lg border border-ix-border/50 flex items-end justify-center pb-2 ${heights[rank]} relative overflow-hidden`}
                  style={{ borderBottom: 'none' }}
                >
                  <div
                    className="absolute inset-0 opacity-10"
                    style={{ background: `linear-gradient(to top, ${c.color || '#6366f1'}, transparent)` }}
                  />
      ...
      <div className="glass rounded-2xl border border-ix-border overflow-hidden">
        ...
        <AnimatePresence mode="popLayout">
          {sorted.map((c, i) => {
            const badge = getRankBadge(c.productivity_score || 0)
            return (
              <motion.div
                key={c.id || c.email || i}
                layout
              ...
              >
                ...
                <div className="col-span-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-ix-muted">{c.team || 'General'}</p>
                  </div>
                </div>
                <div className="col-span-2 hidden sm:block">
                  <div className="flex flex-col items-center gap-1">
                    <span className="text-sm font-display font-bold" style={{ color: c.color || '#6366f1' }}>
                      {c.productivity_score ?? 0}
                    </span>
                    <ProgressBar value={c.productivity_score ?? 0} max={100} color={c.color || '#6366f1'} height={3} />
                  </div>
                </div>
                <div className="col-span-2 text-center hidden md:block">
                  <span className="text-sm font-mono text-ix-text">{c.completed_tasks ?? 0}</span>
                </div>
        ...
        {sorted.length === 0 && (
          <div className="py-12 text-center text-ix-muted text-sm">
      ...
      {lastUpdated && (
        <p className="text-[10px] font-mono text-ix-muted/50 mt-3 text-right">
